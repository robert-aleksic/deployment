...
#    run 'sudo ntpdate ntp.ubuntu.com'
  ...
  task :ruby do
    ...
    run 'mkdir ~/src && cd ~/src && wget -q -nc http://ftp.ruby-lang.org/pub/ruby/1.9/ruby-1.9.3-p286.tar.gz'
    run 'cd ~/src && tar -zxf ruby-1.9.3-p286.tar.gz'
    run 'cd ~/src/ruby-1.9.3-p286 && ./configure && make && sudo make install'
    run 'cd ~ && rm -rf src'
